...
np.random.seed(7)
df=pd.read_csv('DJIA_table.csv',usecols=[4]) # getting data from the file, we are interested in the 4th column; the closing stocks
dataset=df.values
...
model.add(LSTM(256,input_shape=(1,back))) # 1D LSTM model with 10 neurons
...
history=model.fit(X_train,Y_train,epochs=15,batch_size=30,verbose=2)
